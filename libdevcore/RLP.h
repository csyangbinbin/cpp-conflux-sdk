...
static const uint8_t c_rlpDataImmLenCount = c_rlpListStart - c_rlpDataImmLenStart - c_rlpMaxLengthBytes;
...
public:
    ...
    RLP(uint8_t const* _b, unsigned _s, Strictness _st = VeryStrict): RLP(bytesConstRef(_b, _s), _st) {}

    /// Construct a node to read RLP data in the string.
    explicit RLP(std::string const& _s, Strictness _st = VeryStrict): RLP(bytesConstRef((uint8_t const*)_s.data(), _s.size()), _st) {}
