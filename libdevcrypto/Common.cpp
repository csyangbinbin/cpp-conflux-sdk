...
bool toPublicKey(Secret const& _secret, unsigned _flags, array<uint8_t, KeySize>& o_serializedPubkey)
{
    auto* ctx = getCtx();
...
bool ecdh::agree(Secret const& _s, Public const& _r, Secret& o_s) noexcept
{
    ...
    secp256k1_pubkey rawPubkey;
    std::array<uint8_t, 65> serializedPubKey{{0x04}};
    ...
    std::array<uint8_t, 33> compressedPoint;
...
bytes ecies::kdf(Secret const& _z, bytes const& _s1, unsigned kdByteLen)
{
    ...
    std::array<uint8_t, 4> ctr{{0, 0, 0, 1}};
    bytes k;
