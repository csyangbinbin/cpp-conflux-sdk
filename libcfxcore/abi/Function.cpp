...
Data  Function::getSignature(const std::string& func_name) {
    auto hash = Hash::keccak256(Data(func_name.begin(), func_name.end()));
    auto signature = Data(hash.begin(), hash.begin() + 4);
    return signature;
}
...
void Function::encode(Data& data, const std::string& func_name) const  {
    Data signature = getSignature(func_name);
    append(data, signature);
    _inParams.encode(data);
}
