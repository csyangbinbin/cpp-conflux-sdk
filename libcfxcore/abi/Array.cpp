...
int ParamArrayFix::addParam(const std::shared_ptr<ParamBase>& param) {
    assert(param != nullptr);
    if (param == nullptr) { return -1; }
    if (_params.getCount() >= 1 && param->getType() != getFirstType()) { return -2; } // do not add different types
    return _params.addParam(param);
...
std::string ParamArrayFix::getFirstType() const {
    if (_params.getCount() == 0) { return "empty"; }
    return _params.getParamUnsafe(0)->getType();
}

size_t ParamArrayFix::getSize() const{
...
bool ParamArrayFix::isDynamic() const {
    return  _params.getParamUnsafe(0)->isDynamic();
}

std::string ParamArrayFix::getType() const {
    return getFirstType() + "[" +   std::to_string(_params.getCount())  +   "]";
}
